...
void lv_port_disp_init(void)
{
    ...
    // lv_disp_draw_buf_init(&draw_buf_dsc_1, buf_2_1, buf_2_2, LV_HOR_RES_MAX * 10);   /*Initialize the display buffer*/
    ...
    // lv_disp_draw_buf_init(&draw_buf_dsc_1, buf_3_1, buf_3_2, LV_HOR_RES_MAX * LV_VER_RES_MAX);   /*Initialize the display buffer*/
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    for(y = area->y1; y <= area->y2; y++) {
        for(x = area->x1; x <= area->x2; x++) {
            ...
            /*put_px(x, y, *color_p)*/
            screen_put_pixel(x,y,color_p->full);
            // memcpy( + x*pixel_width + y*line_width,
