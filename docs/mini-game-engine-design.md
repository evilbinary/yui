...
**输入**：帧内记录 pressed / released / down；JS 侧 `Game.input.pressed("Space")`。
...
- [ ] `src/game` 最小：time、input 边沿、scene、entity、sprite、camera
- [ ] AABB 可选：无物理也可用重叠检测做拾取
- [ ] `js_game.c` 注册 `Game.*`
- [ ] 挂入现有 tick / render 顺序
- [ ] 示例：`app/game/` 跳跃 + 简易射击竖切
