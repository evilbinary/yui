...
static int esp32_lcd_init(void) {
    ...
    /* 裸 SPI 诊断（临时）：绕开 esp_lcd 面板层，直接对屏发 ST7789
     * 命令并写像素，验证物理 SPI 通路 + 屏型号兼容性。验证后删除。 */
    {
        spi_device_interface_config_t devcfg;
        spi_device_handle_t raw_spi = NULL;
        spi_transaction_t t;
        uint8_t cmd_buf[16];
        ...
        devcfg.queue_size = 4;
        if (spi_bus_add_device((spi_host_device_t)s_cfg.spi_host, &devcfg, &raw_spi) == ESP_OK) {
            int i;
            gpio_set_level((gpio_num_t)s_cfg.dc, 0);   /* DC=0: 命令 */

            /* SWRESET (0x01) */
            memset(&t, 0, sizeof(t));
            t.length = 8; t.tx_buffer = &cmd_buf[0]; cmd_buf[0] = 0x01;
            spi_device_polling_transmit(raw_spi, &t);
            vTaskDelay(pdMS_TO_TICKS(150));
            /* SLPOUT (0x11) */
            memset(&t, 0, sizeof(t));
            t.length = 8; t.tx_buffer = &cmd_buf[0]; cmd_buf[0] = 0x11;
            spi_device_polling_transmit(raw_spi, &t);
            vTaskDelay(pdMS_TO_TICKS(150));
            /* 尝试 3 种像素格式，轮询看哪个出画面 */
#define TEST_COLMOD_VAL 0x55   /* 16bit RGB565 */
            cmd_buf[0] = 0x3A; cmd_buf[1] = TEST_COLMOD_VAL;
            t.length = 16; t.tx_buffer = cmd_buf; spi_device_polling_transmit(raw_spi, &t);
            /* MADCTL: 尝试多种方向 */
            {
                int mad;
                uint8_t px[240 * 2];
                static const uint8_t mads[4] = { 0x00, 0xC0, 0xA0, 0xE0 };
                for (mad = 0; mad < 4; mad++) {
                    cmd_buf[0] = 0x36; cmd_buf[1] = mads[mad];
                    t.length = 16; t.tx_buffer = cmd_buf; spi_device_polling_transmit(raw_spi, &t);
                    /* INVOFF (0x20) */
                    memset(&t, 0, sizeof(t)); t.length = 8; t.tx_buffer = &cmd_buf[0]; cmd_buf[0] = 0x20;
                    spi_device_polling_transmit(raw_spi, &t);
                    /* DISPON */
                    memset(&t, 0, sizeof(t)); t.length = 8; t.tx_buffer = &cmd_buf[0]; cmd_buf[0] = 0x29;
                    spi_device_polling_transmit(raw_spi, &t);
                    vTaskDelay(pdMS_TO_TICKS(30));
                    /* 全屏纯色，轮流 4 色 */
                    {
                        const uint16_t pat[4] = { 0xF800, 0x07E0, 0x001F, 0xFFFF };
                        int p;
                        for (p = 0; p < 4; p++) {
                            uint16_t color = pat[p];
                            uint8_t pix[240 * 2];
                            int y;
                            /* 一次整行 240px * 2 = 480 bytes */
                            gpio_set_level((gpio_num_t)s_cfg.dc, 0);
                            cmd_buf[0] = 0x2A; cmd_buf[1] = 0x00; cmd_buf[2] = 0x00; cmd_buf[3] = 0x00; cmd_buf[4] = 0xEF;
                            t.length = 40; t.tx_buffer = cmd_buf; spi_device_polling_transmit(raw_spi, &t);
                            cmd_buf[0] = 0x2B; cmd_buf[1] = 0x00; cmd_buf[2] = 0x00; cmd_buf[3] = 0x00; cmd_buf[4] = 0xEF;
                            t.length = 40; t.tx_buffer = cmd_buf; spi_device_polling_transmit(raw_spi, &t);
                            cmd_buf[0] = 0x2C;
                            t.length = 8; t.tx_buffer = &cmd_buf[0]; spi_device_polling_transmit(raw_spi, &t);
                            gpio_set_level((gpio_num_t)s_cfg.dc, 1);
                            for (i = 0; i < 240; i++) { px[i*2] = (color >> 8) & 0xFF; px[i*2+1] = color & 0xFF; }
                            for (y = 0; y < 240; y++) {
                                t.length = 240*16; t.tx_buffer = px;
                                spi_device_polling_transmit(raw_spi, &t);
                            }
                            printf("YUI: MADCTL 0x%02x color 0x%04x\n", mads[mad], color);
                            vTaskDelay(pdMS_TO_TICKS(800));
                        }
                    }
                }
            }
            printf("YUI: raw SPI exhaust test done\n");
        } else {
            printf("YUI: raw spi_bus_add_device failed\n");
        }
    }
