...
extern int js_module_resolve_path(const char* in, char* out, size_t out_sz);
...
static JSValue js_list_dir(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv)
{
    ...
    JS_PUSH_VALUE(ctx, arr);
    {
        ...
        while ((de = readdir(dp)) != NULL) {
            ...
            snprintf(names[count], sizeof(names[count]), "%s", leaf);
            if (virt_dir) {
                /* 扁平 FS 的前缀段必然是目录 */
                is_dirs[count] = 1;
            } else {
                /* 真目录：优先 d_type（SPIFFS 填 SPIFFS_TYPE_DIR=2），
                   桌面 FS 用 DT_DIR/DT_UNKNOWN+stat 回退 */
                char path[YUI_MAX_PATH];
                snprintf(path, sizeof(path), "%s/%s", open_dir, leaf);
                int is_dir = 0;
