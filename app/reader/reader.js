...
function loadBook(bookId, page) {
    ...
    // 从文件加载书籍内容
    loadBookFromFile(bookId, page);
}

// 从文件加载书籍内容
function loadBookFromFile(bookId, page) {
    ...
    if (content) {
        ...
        YUI.log("Book split into " + totalPages + " pages");
        
        // 显示指定页的内容
        displayPage(Math.min(page || 1, totalPages));
    } else {
        YUI.log("Failed to load content from file: " + bookPath);
        displayError("无法加载书籍: " + bookId);
    }
}
        ...
        while (remainingText.length > 0) {
            ...
            if (lineCount >= maxLinesPerPage - 2) {
                ...
                if (sentenceEnd > 0) {
                    ...
                    // 分割长句子到多行
                    splitTextIntoLines(sentence, currentPageLines, maxCharsPerLine);
                ...
                splitTextIntoLines(chunk, currentPageLines, maxCharsPerLine);
                lineCount = currentPageLines.length % maxLinesPerPage;
            }
...
// 将文本分割成多行
function splitTextIntoLines(text, linesArray, maxCharsPerLine) {
...
function displayPage(pageNumber) {
    YUI.log("displayPage called with pageNumber: " + pageNumber + ", totalPages: " + totalPages);
    ...
    // 获取章节标题（从第一页提取）
    const chapterTitle = extractChapterTitle(pages[0]);
    ...
    // 更新UI
    updateContent(chapterTitle, pageContent);
...
function updateContent(title, content) {
    ...
    try {
        YUI.setText("bookSelector", currentBook);
...
function updatePageInfo(pageNumber, totalPages) {
    const pageInfoText = `${pageNumber} / ${totalPages}`;
    const pageIndicatorText = `第 ${pageNumber} 页，共 ${totalPages} 页`;
...
// 更新进度
function updateProgress(pageNumber, totalPages) {
...
function saveReadingProgress() {
    YUI.log("Saving reading progress for: " + currentBook + ", page: " + currentPage);
    // 在实际应用中，这里应该保存到本地存储
}
...
if (typeof global !== 'undefined') {
    ...
    global.goToPrevPage = goToPrevPage;
    global.goToNextPage = goToNextPage;
    global.toggleTheme = toggleTheme;
    global.decreaseFont = decreaseFont;
    global.increaseFont = increaseFont;
    global.toggleDebug = toggleDebug;
}
