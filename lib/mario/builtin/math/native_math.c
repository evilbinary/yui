...
//Math.floor(a) - returns the largest integer less than or equal to a given number
var_t* native_math_floor(vm_t* vm, var_t* env, void *data) {
	...
	if (n->var->type == V_FLOAT)
		return var_new_int(vm, (int)floor(var_get_float(n->var)));
	return var_new_int(vm, var_get_int(n->var));
}

//Math.ceil(a) - returns the smallest integer greater than or equal to a given number
var_t* native_math_ceil(vm_t* vm, var_t* env, void *data) {
	...
	if (n->var->type == V_FLOAT)
		return var_new_int(vm, (int)ceil(var_get_float(n->var)));
	return var_new_int(vm, var_get_int(n->var));
}
