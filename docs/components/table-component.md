...
Table 组件用于展示多列结构化数据，支持表头、斑马纹、网格线、双向滚动、行选择与 `onSelect` 事件。适用于查询结果、数据面板等场景。
...
| `editable` | boolean | `true` | 是否允许编辑单元格 |
...
|------|------|
| 单击单元格 | 选中行与列 |
| 双击 / F2 / Enter | 进入编辑并全选内容（`editable: true` 时） |
| 直接输入 | 替换当前单元格内容并进入编辑 |
| Escape | 取消编辑 |
| Enter | 提交编辑 |
| ← / → | 移动光标（按字符）；Shift+←/→ 扩展选区 |
| 鼠标拖选 | 在编辑单元格内按住左键拖动选择 |
| Home / End | 光标到行首/行尾；Shift 扩展选区 |
| Ctrl+A | 全选 |
| Tab / Shift+Tab | 编辑中提交并移到相邻列 |
| 方向键 | 未编辑时移动单元格选中 |
| Ctrl+C | 复制当前单元格到剪贴板 |
| Ctrl+Shift+C | 复制当前行（制表符分隔） |
| Ctrl+V | 编辑中粘贴 |

编辑结果写回 `layer.data` 对应行的 JSON 字段；数值列在可解析为数字时保持 number 类型。

## 事件

### onSelect

选中数据行时触发（表头、滚动条区域不触发）。
...
引擎在触发事件前将行 JSON 写入 `layer.text`，与 List、Treeview 的项选中模式一致。
